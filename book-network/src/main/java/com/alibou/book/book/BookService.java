...
import com.alibou.book.history.TranactionHistoryRepository;
...
public class BookService {
    ...
    private final TranactionHistoryRepository tranactionHistoryRepository;
    ...
    public PageResponse<BookResponse> findAllBooks(final int page, final int size, final Authentication connectedUser) {
        User user = ((User) connectedUser.getPrincipal());
    ...
    public PageResponse<BorrowedBookResponse> findAllBorrowebBooks(final int page, final int size, final Authentication connectedUser) {
        User user = ((User) connectedUser.getPrincipal());
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
        Page<BookTransactionHistory> allBorrowedBooks = tranactionHistoryRepository.findAllBorrowedBooks(pageable, user.getId());
    ...
    public PageResponse<BorrowedBookResponse> findAllReturnedBooks(final int page, final int size, final Authentication connectedUser) {
        User user = ((User) connectedUser.getPrincipal());
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
        Page<BookTransactionHistory> allBorrowedBooks = tranactionHistoryRepository.findAllReturnedBooks(pageable, user.getId());
        List<BorrowedBookResponse> bookResponse = allBorrowedBooks.stream()
                .map(bookMapper::toBorrowedBookResponse)
                .toList();
    ...
    public Integer borrowBook(final Integer bookId, final Authentication connectedUser) {
        ...
        final boolean isAlreadyBorrowed = tranactionHistoryRepository.isAlreadyBorrowedByUser(bookId, user.getId());
        ...
        BookTransactionHistory bookTransactionHistory = BookTransactionHistory.builder()
                ...
                .build();
        return tranactionHistoryRepository.save(bookTransactionHistory).getId();
    }

    public Integer returnBorrowedBook(final Integer bookId, final Authentication connectedUser) {
        Book book = bookRepository.findById(bookId)
                .orElseThrow(() -> new EntityNotFoundException("No book found with the ID::"+bookId));
        if (book.isArchived() || !book.isShareable()){
            throw new OperationNotPermittedException("The requested book cannot be borrowed since it archived or not shareable.");
        ...
        BookTransactionHistory bookTransactionHistory = tranactionHistoryRepository.findByBookIdAndUserId(bookId, user.getId())
    ...
    public Integer approveReturnBorrowedBook(final Integer bookId, final Authentication connectedUser) {
        ...
        BookTransactionHistory bookTransactionHistory = tranactionHistoryRepository.findByBookIdAndOwnerId(bookId, user.getId())
